...
#rec_equ="d10(0,_n2,_n3,_n4,b,e,d) = d26(_n2,_n3,_n4,b,e,d);e10(0,_n2,_n3,_n4,b,e,d) = 1;e10((_n1+1),_n2,_n3,_n4,b,e,d) = ite((d10(_n1,_n2,_n3,_n4,b,e,d)>=e10(_n1,_n2,_n3,_n4,b,e,d)),(e10(_n1,_n2,_n3,_n4,b,e,d)+1),e10(_n1,_n2,_n3,_n4,b,e,d));d10((_n1+1),_n2,_n3,_n4,b,e,d) = ite((d10(_n1,_n2,_n3,_n4,b,e,d)>=e10(_n1,_n2,_n3,_n4,b,e,d)),d10(_n1,_n2,_n3,_n4,b,e,d),ite((e10(_n1,_n2,_n3,_n4,b,e,d)>=(d10(_n1,_n2,_n3,_n4,b,e,d)+1)),(d10(_n1,_n2,_n3,_n4,b,e,d)+1),d10(_n1,_n2,_n3,_n4,b,e,d)))"
...
def solve_recurrence_call(list_equations,var):
    ...
    for result in results:
        if result[0]=='i0' or result[0]=='i1':
            if list_para is None and len( result[-2])>2:
                list_para=[]
                for x in result[-2][2:]:
                    list_para.append(x)
            list_equations.append(result)
    ...
    if len(list_equations)>0:
        ...
        if axoims is not None and list_para is not None:  
            list_values=[] 
            for axoim in axoims:
                if axoim[0]=='s1':
                    temp_list_values=[]
                    temp_list_values.append(axoim[1][1][2][2])
                    temp_list_values.append(axoim[1][1][2][2]+list_para)
                    
                    list_values.append(temp_list_values)
        
            for axoim in axoims:
                for x in list_values:
                    axoim[-1]=fun_utiles.expr_replace(axoim[-1], x[0], x[1])
            for y in closed_formed:
                for x in list_values:
                    y[-1]=fun_utiles.expr_replace(y[-1], x[0], x[1])
    
    return closed_formed, axoims
...
def constructInfoSystem(e, equations_map, basecase_map, list_equations):
    ...
    for x in e[6]:
        ...
        new_e=eval("['"+e[1][0]+"','"+str(e[1][1])+"','"+e[1][2]+"',"+str(e[1][3])+","+str(e[6][x][1])+"]")
...
def constructInfoSystemGroupFunctionDifCon(group_list, equations_map, basecase_map, list_equations):
    ...
    for x in group_list:
        ...
        #for y in x[-1]:
            ...
            if y in x[-1].keys():
                ...
                new_xy=copy.deepcopy(x[1][-2])
                
                new_xy = fun_utiles.expr_replace(new_xy,eval("['+',['"+x[1][2]+"'],['1']]"),eval("['"+x[1][2]+"']"))
                
                temp_list1.append(condition_map_temp1[y])
                
                temp_list1.append(new_xy)
                
                temp_list1.append(condition_map_temp[y])
...
def solveGroupFunctionType(group_list, equations_map, basecase_map, list_equations):
    ...
    if status is True:
        ...
        condition_map = constructInfoSystemGroupFunctionDifCon(group_list, equations_map, basecase_map, list_equations)
...
def solve_rec(e1,e2):
    ...
    if e1[0]=='i1':
                ret_value = fun_utiles.isBigNPresent(e1[-1])
                if ret_value is not None:
                    e_new = copy.deepcopy(e1)
                    e_new[-1] = fun_utiles.recSummation(e_new[-1])
                    if '_SUMMATION' in FOL_translation.expr2string1(e_new[-1]):
                        e_new[-2] = fun_utiles.expr_replace(e_new[-2],eval("['+',['"+e_new[-3]+"'],['1']]"),eval("['"+e_new[-3]+"']"))
                        e_new[-1] = fun_utiles.expr_replace(e_new[-1],e_new[-2],e2[-1])
                        e_new[0] = 'i2'
                        return e_new
                        #return None
                    else:
                        ret_value = fun_utiles.isExpressConstant(e1[-1],  eval("['"+e1[-3]+"']"))
    ...
    if variable is not None and lefthandstmt is not None and righthandstmt is not None and lefthandstmt_base is not None and righthandstmt_base is not None:
        ...
        if fun_utiles.is_number(str(righthandstmt))==True:
            expression=str(str(term2)+"="+str(righthandstmt))
            utiles_translation.resetGlobal()
