...
class SLexer(object):
	...
	def filterSyntax(self):
		...
		if tok.type=='SCONCATSPACE':
                   ...
                   if flgCheck == True and tok.type=='SEMICOLON':
                      ...
                      for mTok in temp_list:

                          if mTok.type=='ASSIGN':

                             fst_flag = True

                             new_Temp.append(mTok)

                          elif mTok.type=='LSQUARE':

                             snd_flag = True

                          elif mTok.type=='RSQUARE':

                             trd_flag = True

                          elif fst_flag == True and snd_flag == True and trd_flag == True :

                              trd_para.append(mTok)


                          elif fst_flag == True and snd_flag == True and trd_flag == False :

                              snd_para.append(mTok)


                          elif fst_flag == True and snd_flag == False and trd_flag == False :

                              fst_para.append(mTok)

                          elif fst_flag == False and snd_flag == False and trd_flag == False :
                               
                              new_Temp.append(mTok)


                      fname_mTok = copy.deepcopy(new_Temp[1])

                      fname_mTok.type = 'ID'

                      fname_mTok.value = 'W_'

                      fname_mTok.lineno = new_Temp[0].lineno

                      fname_mTok.lexpos = new_Temp[0].lexpos

                      new_Temp.append(fname_mTok)

                      p1name_mTok = copy.deepcopy(new_Temp[1])

                      p1name_mTok.type = 'LPAREN'

                      p1name_mTok.value = '('

                      new_Temp.append(p1name_mTok)

                      new_Temp = new_Temp + snd_para

                      c1name_mTok = copy.deepcopy(new_Temp[1])

                      c1name_mTok.type = 'COMMA'

                      c1name_mTok.value = ','

                      new_Temp.append(c1name_mTok)

                      new_Temp = new_Temp + fst_para

                      c2name_mTok = copy.deepcopy(new_Temp[1])

                      c2name_mTok.type = 'COMMA'

                      c2name_mTok.value = ','

                      new_Temp.append(c2name_mTok)

                      new_Temp = new_Temp + trd_para[:-1]

                      p2name_mTok = copy.deepcopy(new_Temp[1])

                      p2name_mTok.type = 'RPAREN'

                      p2name_mTok.value = ')'

                      new_Temp.append(p2name_mTok)

                      main_list = main_list+new_Temp
                   ...
                   if flgCheck == True:
                      ...
                      main_list.append(tok)

            #print(self.showProgram(main_list)) 
            return  main_list               
