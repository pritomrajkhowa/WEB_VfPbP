...
def isBigNPresent(e):
    ...
    if len(args)==0:
        ...
        if op=='and' or op=='or':
            ...
            if '_N' in op:
                return e
            elif '_SUMMATION' in op:
                return e
            elif '__VERIFIER_nondet_' in op:
                return e
            elif '_CE' in op:
                return e
...
def recSummation(e):
    ...
    if len(args)==0:
        ...
        if op=='and' or op=='or':
            return FOL_translation.expres(op, recSummation(args[0]), recSummation(args[1]))
        elif op=='not' and len(args)==1:
            return FOL_translation.expres(op, recSummation(args[0]))
        elif op=='implies' and len(args)==2:
            list_temp=[]
            list_temp.append(recSummation(args[0]))
            list_temp.append(recSummation(args[1]))
            return FOL_translation.expres(op, list_temp)
        elif op == '-' and len(args)==1:
            return FOL_translation.expres(op, recSummation(args[0]))
            ...
            list_temp=[]
            list_temp.append(args[0])
            list_temp.append(args[1])
            return FOL_translation.expres(op, list_temp)
        else:
            if '_N' in op:
                list_temp=[]
                list_temp.append(e)
                return FOL_translation.expres('_SUMMATION', list_temp)
            elif '__VERIFIER_nondet_' in op:
                list_temp=[]
                list_temp.append(e)
                return FOL_translation.expres('_SUMMATION', list_temp)
...
def isExpressConstant(e, var):
    args=FOL_translation.expr_args(e)
    op= FOL_translation.expr_op(e)
    if op=='and' or op=='or':
        if len(args)==1:
            result = isExpressConstant(args[0], var)
            if result is not None:
                return result
        else:
            result = isExpressConstant(args[0], var)
            if result is not None:
                return result
            result = isExpressConstant(args[1], var)
            if result is not None:
                return result
    elif op=='not' and len(args)==1:
            result = isExpressConstant(args[0], var)
        ...
        if '_N' not in op and '_SUMMATION' not in op and '_CE' not in op and '__VERIFIER_nondet_' not in op and len(args)>0:
            if var in args:
                return e
            else:
                return None
...
def isNonDeterminstic(e):
    args=FOL_translation.expr_args(e)
    op= FOL_translation.expr_op(e)
    if len(args)==0:
        if '__VERIFIER_nondet_' in op:
            return e
    else:
        if op=='and' or op=='or':
            if len(args)==1:
                result = isNonDeterminstic(args[0])
                if result is not None:
                    return result
            else:
                result = isNonDeterminstic(args[0])
                if result is not None:
                    return result
                result = isNonDeterminstic(args[1])
                if result is not None:
                    return result
        elif op=='not' and len(args)==1:
                result = isNonDeterminstic(args[0])
